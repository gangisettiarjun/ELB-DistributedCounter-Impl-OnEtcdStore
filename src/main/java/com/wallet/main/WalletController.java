...
import com.wallet.beans.BankAccount;
import com.wallet.beans.IDCard;
import com.wallet.beans.User;
import com.wallet.beans.WebLogin;
import com.wallet.exception.ErrorMessage;
...
public class WalletController{
    ...
    public int getCounter() throws EtcdClientException {
        EtcdClient client = new EtcdClient(URI.create("http://54.67.123.226:4001/"));
        String key = "/010010039/counter";
        System.out.println("Your Key is" +key);
        EtcdResult result= null;
        int counter=0;
        try{
        	result = client.get(key);
        	counter = Integer.parseInt(result.node.value);
        	System.out.println("Counter Value prior to Increment: "+ counter);
        	counter++;
        }
        catch(Exception ex){
        	...
        	counter = Integer.parseInt(result.node.value);
        	System.out.println("Counter Value After the Increment: "+ counter);
        }
        return counter;
    }
